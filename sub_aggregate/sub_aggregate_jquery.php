...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["idaggregate_" + iSeqRow] && scEventControl_data["idaggregate_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function sc_sub_aggregate_idaggregate__onblur(oThis, iSeqRow) {
  do_ajax_sub_aggregate_validate_idaggregate_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_sub_aggregate_idaggregate__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_sub_aggregate_idaggregate__onfocus(oThis, iSeqRow) {
...
function sc_sub_aggregate_idholder__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}
...
function displayChange_field_idaggregate_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_sub_aggregate_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'sub_aggregate')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
