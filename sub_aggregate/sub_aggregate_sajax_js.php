  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_sub_aggregate" == sTestField)
        {
...
?>
  ...
  {
    var nomeCampo_idaggregate_ = "idaggregate_" + iNumLinha;
    var var_idaggregate_ = scAjaxGetFieldHidden(nomeCampo_idaggregate_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_sub_aggregate_validate_idaggregate_(var_idaggregate_, iNumLinha, var_script_case_init, do_ajax_sub_aggregate_validate_idaggregate__cb);
  } // do_ajax_sub_aggregate_validate_idaggregate_

  function do_ajax_sub_aggregate_validate_idaggregate__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "idaggregate_" + iLineNumber;
    }
    else
    {
      sFieldValid = "idaggregate_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "idaggregate_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "idaggregate_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_sub_aggregate_validate_idaggregate__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_sub_aggregate_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_sub_aggregate_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_sub_aggregate_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_sub_aggregate_add_new_line_cb);
  } // do_ajax_sub_aggregate_add_new_line

  function do_ajax_sub_aggregate_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
    ...
    scFixCol_loadState();
  } // do_ajax_sub_aggregate_add_new_line_cb

  // ---------- backup_line
  function do_ajax_sub_aggregate_backup_line(iNumLinha)
  {
    var var_idaggregate_ = scAjaxGetFieldHidden("idaggregate_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_sub_aggregate_backup_line(var_idaggregate_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_sub_aggregate_backup_line_cb);
  } // do_ajax_sub_aggregate_backup_line

  function do_ajax_sub_aggregate_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_sub_aggregate_backup_line_cb

  function do_ajax_sub_aggregate_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_sub_aggregate_cancel_insert

  function do_ajax_sub_aggregate_cancel_update(iSeqVert)
  {
    do_ajax_sub_aggregate_backup_line(iSeqVert);
...
?>
  } // do_ajax_sub_aggregate_cancel_update

  function do_ajax_sub_aggregate_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_sub_aggregate_restore_buttons

  // ---------- table_refresh
  function do_ajax_sub_aggregate_table_refresh()
  {
    ...
    scAjaxProcOn();
    x_ajax_sub_aggregate_table_refresh(var_idaggregate_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_sub_aggregate_table_refresh_cb);
  } //  do_ajax_sub_aggregate_table_refresh

  function do_ajax_sub_aggregate_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    document.F2.idaggregate_.value = scAjaxGetKeyValue("idaggregate_");
    ...
    scAjaxAlert(do_ajax_sub_aggregate_table_refresh_cb_after_alert);
  } // do_ajax_sub_aggregate_table_refresh_cb
  function do_ajax_sub_aggregate_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxSetNavStatus("b");
    sc_insert_open = false;
  } // do_ajax_sub_aggregate_table_refresh_cb_after_alert
...
function scJs_sweetalert_params(params) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_sub_aggregate_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_sub_aggregate_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_sub_aggregate_submit_form_cb_after_alert);
  } // do_ajax_sub_aggregate_submit_form_cb
  function do_ajax_sub_aggregate_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_sub_aggregate_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_sub_aggregate_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_sub_aggregate_navigate_form, function() {});
    } else {
      perform_ajax_sub_aggregate_navigate_form();
    }
  }

  function perform_ajax_sub_aggregate_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    if (sc_insert_open)
    {
        do_ajax_sub_aggregate_cancel_insert(sc_sub_aggregate_get_last_line_number());
    }
    nm_uncheck_delete();
    scAjaxHideMessage();
    scAjaxHideErrorDisplay("table");
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_sub_aggregate_navigate_form(var_idaggregate_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_sub_aggregate_navigate_form_cb);
  } // do_ajax_sub_aggregate_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['sub_aggregate']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_sub_aggregate_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.idaggregate_.value = scAjaxGetKeyValue("idaggregate_" + var_last_index);
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_sub_aggregate_navigate_form_cb_after_alert);
  } // do_ajax_sub_aggregate_navigate_form_cb
  function do_ajax_sub_aggregate_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
?>
  ...
  } // do_ajax_sub_aggregate_navigate_form_cb_after_alert

    function sc_sub_aggregate_get_last_line_number()
    {
    ...
    } //sc_sub_aggregate_get_last_line_number

  function sc_hide_sub_aggregate_form()
  {
...
?>
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ...
      ajax_error_list["idaggregate_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["idaggregate_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
?>
    scAjaxFieldRead("idaggregate_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }
