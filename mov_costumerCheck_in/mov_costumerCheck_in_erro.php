...
$old_error_handler = set_error_handler("mov_costumerCheck_in_scriptcase_error_handler");

function mov_costumerCheck_in_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['mov_costumerCheck_in']['contr_erro']) && $_SESSION['scriptcase']['mov_costumerCheck_in']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_mov_costumerCheck_in->contr_mov_costumerCheck_in->Apl_com_erro = true;
            if ($inicial_mov_costumerCheck_in->contr_mov_costumerCheck_in->NM_ajax_flag)
            {
                $inicial_mov_costumerCheck_in->contr_mov_costumerCheck_in->NM_ajax_info['errList']['geral_mov_costumerCheck_in'][] = $msg;
                return;
            }
            mov_costumerCheck_in_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function mov_costumerCheck_in_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['mov_costumerCheck_in'])  && '' != $_SESSION['scriptcase']['error_icon']['mov_costumerCheck_in'])  ? $_SESSION['scriptcase']['error_icon']['mov_costumerCheck_in']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['mov_costumerCheck_in']) && '' != $_SESSION['scriptcase']['error_close']['mov_costumerCheck_in']) ? $_SESSION['scriptcase']['error_close']['mov_costumerCheck_in'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['mov_costumerCheck_in']) && '' != $_SESSION['scriptcase']['error_span_title']['mov_costumerCheck_in']) ? $_SESSION['scriptcase']['error_span_title']['mov_costumerCheck_in'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['mov_costumerCheck_in']) && '' != $_SESSION['scriptcase']['error_icon_title']['mov_costumerCheck_in']) ? $_SESSION['scriptcase']['error_icon_title']['mov_costumerCheck_in'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['mov_costumerCheck_in']['error_buffer'] .= ob_get_flush();
}
