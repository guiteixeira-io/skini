...
function scEventControl_init(iSeqRow) {
  scEventControl_data["idcostumercheck_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["idcostumer_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["idlodge_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["sc_field_0_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["idcostumercheck_" + iSeqRow] && scEventControl_data["idcostumercheck_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["idcostumercheck_" + iSeqRow] && scEventControl_data["idcostumercheck_" + iSeqRow]["change"]) {
    return true;
  }
  ...
  if (scEventControl_data["idlodge_" + iSeqRow] && scEventControl_data["idlodge_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["idlodge_" + iSeqRow] && scEventControl_data["idlodge_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["sc_field_0_" + iSeqRow] && scEventControl_data["sc_field_0_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["sc_field_0_" + iSeqRow] && scEventControl_data["sc_field_0_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_idcostumercheck_' + iSeqRow).bind('blur', function() { sc_mov_costumerCheck_in_idcostumercheck__onblur(this, iSeqRow) })
                                              .bind('change', function() { sc_mov_costumerCheck_in_idcostumercheck__onchange(this, iSeqRow) })
                                              .bind('focus', function() { sc_mov_costumerCheck_in_idcostumercheck__onfocus(this, iSeqRow) });
  $('#id_sc_field_idcostumer_' + iSeqRow).bind('blur', function() { sc_mov_costumerCheck_in_idcostumer__onblur(this, iSeqRow) })
                                         .bind('change', function() { sc_mov_costumerCheck_in_idcostumer__onchange(this, iSeqRow) })
                                         .bind('focus', function() { sc_mov_costumerCheck_in_idcostumer__onfocus(this, iSeqRow) });
  $('#id_sc_field_idlodge_' + iSeqRow).bind('blur', function() { sc_mov_costumerCheck_in_idlodge__onblur(this, iSeqRow) })
                                      .bind('change', function() { sc_mov_costumerCheck_in_idlodge__onchange(this, iSeqRow) })
                                      .bind('focus', function() { sc_mov_costumerCheck_in_idlodge__onfocus(this, iSeqRow) });
  $('#id_sc_field_sc_field_0_' + iSeqRow).bind('blur', function() { sc_mov_costumerCheck_in_sc_field_0__onblur(this, iSeqRow) })
                                         .bind('change', function() { sc_mov_costumerCheck_in_sc_field_0__onchange(this, iSeqRow) })
                                         .bind('focus', function() { sc_mov_costumerCheck_in_sc_field_0__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_mov_costumerCheck_in_idcostumercheck__onblur(oThis, iSeqRow) {
  do_ajax_mov_costumerCheck_in_validate_idcostumercheck_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_mov_costumerCheck_in_idcostumercheck__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_mov_costumerCheck_in_idcostumercheck__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_mov_costumerCheck_in_idcostumer__onblur(oThis, iSeqRow) {
  do_ajax_mov_costumerCheck_in_validate_idcostumer_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_mov_costumerCheck_in_idcostumer__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_mov_costumerCheck_in_idcostumer__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_mov_costumerCheck_in_idlodge__onblur(oThis, iSeqRow) {
  do_ajax_mov_costumerCheck_in_validate_idlodge_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_mov_costumerCheck_in_idlodge__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_mov_costumerCheck_in_idlodge__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_mov_costumerCheck_in_sc_field_0__onblur(oThis, iSeqRow) {
  do_ajax_mov_costumerCheck_in_validate_sc_field_0_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_mov_costumerCheck_in_sc_field_0__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_mov_costumerCheck_in_sc_field_0__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("idcostumercheck_", "", status);
	displayChange_field("idcostumer_", "", status);
	displayChange_field("idlodge_", "", status);
	displayChange_field("sc_field_0_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_idcostumercheck_(row, status);
	displayChange_field_idcostumer_(row, status);
	displayChange_field_idlodge_(row, status);
	displayChange_field_sc_field_0_(row, status);
}

function displayChange_field(field, row, status) {
	if ("idcostumercheck_" == field) {
		displayChange_field_idcostumercheck_(row, status);
	}
...
function displayChange_field_idcostumercheck_(row, status) {
    var fieldId;
}
...
function displayChange_field_idcostumer_(row, status) {
	...
	if ("on" == status) {
		if ("all" == row) {
			var fieldList = $(".css_idcostumer___obj");
			for (var i = 0; i < fieldList.length; i++) {
				$($(fieldList[i]).attr("id")).select2("destroy");
			}
		}
		else {
			$("#id_sc_field_idcostumer_" + row).select2("destroy");
		}
		scJQSelect2Add(row, "idcostumer_");
	}
}

function displayChange_field_idlodge_(row, status) {
    var fieldId;
	if ("on" == status) {
		if ("all" == row) {
			var fieldList = $(".css_idlodge___obj");
			for (var i = 0; i < fieldList.length; i++) {
				$($(fieldList[i]).attr("id")).select2("destroy");
			}
		}
		else {
			$("#id_sc_field_idlodge_" + row).select2("destroy");
		}
		scJQSelect2Add(row, "idlodge_");
	}
}
...
function scRecreateSelect2() {
	displayChange_field_idcostumer_("all", "on");
	displayChange_field_idlodge_("all", "on");
}
...
function scHidePage(pageNo) {
	$("#id_mov_costumerCheck_in_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'mov_costumerCheck_in')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
...
function scJQSelect2Add(seqRow, specificField) {
  if (null == specificField || "idcostumer_" == specificField) {
    scJQSelect2Add_idcostumer_(seqRow);
  }
  if (null == specificField || "idlodge_" == specificField) {
    scJQSelect2Add_idlodge_(seqRow);
  }
} // scJQSelect2Add

function scJQSelect2Add_idcostumer_(seqRow) {
  var elemSelector = "all" == seqRow ? ".css_idcostumer__obj" : "#id_sc_field_idcostumer_" + seqRow;
  $(elemSelector).select2(
    {
      containerCssClass: 'css_idcostumer__obj',
      dropdownCssClass: 'css_idcostumer__obj',
      language: {
        noResults: function() {
...
function scJQSelect2Add_idlodge_(seqRow) {
  var elemSelector = "all" == seqRow ? ".css_idlodge__obj" : "#id_sc_field_idlodge_" + seqRow;
  $(elemSelector).select2(
    {
      containerCssClass: 'css_idlodge__obj',
      dropdownCssClass: 'css_idlodge__obj',
      language: {
        noResults: function() {
          return "<?php echo $this->Ini->Nm_lang['lang_autocomp_notfound'] ?>";
        },
...
function scJQElementsAdd(iLine) {
  ...
  setTimeout(function () { if ('function' == typeof displayChange_field_idcostumer_) { displayChange_field_idcostumer_(iLine, "on"); } }, 150);
  setTimeout(function () { if ('function' == typeof displayChange_field_idlodge_) { displayChange_field_idlodge_(iLine, "on"); } }, 150);
} // scJQElementsAdd
