...
?>
  ...
  } // do_ajax_cad_lodge_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_cad_lodge_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_cad_lodge_navigate_form, function() {});
    } else {
      perform_ajax_cad_lodge_navigate_form();
    }
  }

  function perform_ajax_cad_lodge_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
...
?>
  function do_ajax_cad_lodge_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
