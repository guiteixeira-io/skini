...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_list_groups'] ?></TITLE>
...
<?php
   ...
   {
      if ($this->Ini->Embutida_iframe || $_SESSION['sc_session'][$this->Ini->sc_page]['sec_search_sec_groups']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_list_groups'] ?></div>
...
<?php
   ...
   function monta_form()
   {
      ...
      {
         $tmp_pos = (is_string($group_id)) ? strpos($group_id, "##@@") : false;
         if ($tmp_pos === false)
         { }
         else
         {
...
<?php
      $group_id_look = (is_string($group_id) ? substr($this->Db->qstr($group_id), 1, -1) : $group_id); 
      $nmgp_def_dados = "" ; 
...
?>
   ...
   <INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<?php
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_search_sec_groups']['orig_pesq']) && $_SESSION['sc_session'][$this->Ini->sc_page]['sec_search_sec_groups']['orig_pesq'] == "grid")
   {
...
$_SESSION['scriptcase']['sec_search_sec_groups']['contr_erro'] = 'off'; 
      ...
      if (isset($group_id))
      {
         $this->monta_condicao("group_id", $group_id_cond, $group_id, "", "group_id", "INT", false);
      }
   }
