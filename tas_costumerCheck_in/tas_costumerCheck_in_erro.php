...
$old_error_handler = set_error_handler("tas_costumerCheck_in_scriptcase_error_handler");

function tas_costumerCheck_in_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['tas_costumerCheck_in']['contr_erro']) && $_SESSION['scriptcase']['tas_costumerCheck_in']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_tas_costumerCheck_in->contr_tas_costumerCheck_in->Apl_com_erro = true;
            if ($inicial_tas_costumerCheck_in->contr_tas_costumerCheck_in->NM_ajax_flag)
            {
                $inicial_tas_costumerCheck_in->contr_tas_costumerCheck_in->NM_ajax_info['errList']['geral_tas_costumerCheck_in'][] = $msg;
                return;
            }
            tas_costumerCheck_in_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function tas_costumerCheck_in_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['tas_costumerCheck_in'])  && '' != $_SESSION['scriptcase']['error_icon']['tas_costumerCheck_in'])  ? $_SESSION['scriptcase']['error_icon']['tas_costumerCheck_in']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['tas_costumerCheck_in']) && '' != $_SESSION['scriptcase']['error_close']['tas_costumerCheck_in']) ? $_SESSION['scriptcase']['error_close']['tas_costumerCheck_in'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['tas_costumerCheck_in']) && '' != $_SESSION['scriptcase']['error_span_title']['tas_costumerCheck_in']) ? $_SESSION['scriptcase']['error_span_title']['tas_costumerCheck_in'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['tas_costumerCheck_in']) && '' != $_SESSION['scriptcase']['error_icon_title']['tas_costumerCheck_in']) ? $_SESSION['scriptcase']['error_icon_title']['tas_costumerCheck_in'] : '';
?>
