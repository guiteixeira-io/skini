...
$old_error_handler = set_error_handler("cad_costumers_scriptcase_error_handler");

function cad_costumers_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['cad_costumers']['contr_erro']) && $_SESSION['scriptcase']['cad_costumers']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_cad_costumers->contr_cad_costumers->Apl_com_erro = true;
            if ($inicial_cad_costumers->contr_cad_costumers->NM_ajax_flag)
            {
                $inicial_cad_costumers->contr_cad_costumers->NM_ajax_info['errList']['geral_cad_costumers'][] = $msg;
                return;
            }
            cad_costumers_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function cad_costumers_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['cad_costumers'])  && '' != $_SESSION['scriptcase']['error_icon']['cad_costumers'])  ? $_SESSION['scriptcase']['error_icon']['cad_costumers']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['cad_costumers']) && '' != $_SESSION['scriptcase']['error_close']['cad_costumers']) ? $_SESSION['scriptcase']['error_close']['cad_costumers'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['cad_costumers']) && '' != $_SESSION['scriptcase']['error_span_title']['cad_costumers']) ? $_SESSION['scriptcase']['error_span_title']['cad_costumers'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['cad_costumers']) && '' != $_SESSION['scriptcase']['error_icon_title']['cad_costumers']) ? $_SESSION['scriptcase']['error_icon_title']['cad_costumers'] : '';
?>
