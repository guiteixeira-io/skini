...
$old_error_handler = set_error_handler("sub_lodgePrice_scriptcase_error_handler");

function sub_lodgePrice_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['sub_lodgePrice']['contr_erro']) && $_SESSION['scriptcase']['sub_lodgePrice']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_sub_lodgePrice->contr_sub_lodgePrice->Apl_com_erro = true;
            if ($inicial_sub_lodgePrice->contr_sub_lodgePrice->NM_ajax_flag)
            {
                $inicial_sub_lodgePrice->contr_sub_lodgePrice->NM_ajax_info['errList']['geral_sub_lodgePrice'][] = $msg;
                return;
            }
            sub_lodgePrice_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function sub_lodgePrice_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['sub_lodgePrice'])  && '' != $_SESSION['scriptcase']['error_icon']['sub_lodgePrice'])  ? $_SESSION['scriptcase']['error_icon']['sub_lodgePrice']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['sub_lodgePrice']) && '' != $_SESSION['scriptcase']['error_close']['sub_lodgePrice']) ? $_SESSION['scriptcase']['error_close']['sub_lodgePrice'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['sub_lodgePrice']) && '' != $_SESSION['scriptcase']['error_span_title']['sub_lodgePrice']) ? $_SESSION['scriptcase']['error_span_title']['sub_lodgePrice'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['sub_lodgePrice']) && '' != $_SESSION['scriptcase']['error_icon_title']['sub_lodgePrice']) ? $_SESSION['scriptcase']['error_icon_title']['sub_lodgePrice'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['sub_lodgePrice']['error_buffer'] .= ob_get_flush();
}
