<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idcostumer_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idcostumer_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="tas_costumer_checkout"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['tas_costumer_checkout']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['tas_costumer_checkout']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['tas_costumer_checkout']['dashboard_info']['parent_widget'] . "']\")";
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_tas_costumer_checkout_submit_form(y, z);
...
var nm_num_campo = 1;
function lookup_lodge_(campo)
{
   campo = (campo != null) ? campo : nm_num_campo;
   nm_num_campo = campo;
   dados  = eval('document.F1.idlodge_' + campo + '.value');
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('tas_costumer_checkout')?>/tas_costumer_checkout.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_lodge_, "lodge_", dados, false);
}
function recebe_lodge_(string)
{
   document.getElementById('id_lookup_lodge_' + nm_num_campo).innerHTML = string;
}
<?php 
