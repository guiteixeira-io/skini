...
$old_error_handler = set_error_handler("tas_costumer_checkout_scriptcase_error_handler");

function tas_costumer_checkout_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['tas_costumer_checkout']['contr_erro']) && $_SESSION['scriptcase']['tas_costumer_checkout']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_tas_costumer_checkout->contr_tas_costumer_checkout->Apl_com_erro = true;
            if ($inicial_tas_costumer_checkout->contr_tas_costumer_checkout->NM_ajax_flag)
            {
                $inicial_tas_costumer_checkout->contr_tas_costumer_checkout->NM_ajax_info['errList']['geral_tas_costumer_checkout'][] = $msg;
                return;
            }
            tas_costumer_checkout_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function tas_costumer_checkout_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['tas_costumer_checkout'])  && '' != $_SESSION['scriptcase']['error_icon']['tas_costumer_checkout'])  ? $_SESSION['scriptcase']['error_icon']['tas_costumer_checkout']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['tas_costumer_checkout']) && '' != $_SESSION['scriptcase']['error_close']['tas_costumer_checkout']) ? $_SESSION['scriptcase']['error_close']['tas_costumer_checkout'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['tas_costumer_checkout']) && '' != $_SESSION['scriptcase']['error_span_title']['tas_costumer_checkout']) ? $_SESSION['scriptcase']['error_span_title']['tas_costumer_checkout'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['tas_costumer_checkout']) && '' != $_SESSION['scriptcase']['error_icon_title']['tas_costumer_checkout']) ? $_SESSION['scriptcase']['error_icon_title']['tas_costumer_checkout'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['tas_costumer_checkout']['error_buffer'] .= ob_get_flush();
}
