<?php
   ...
   if(!$bol_sel_campos_include)
   {
     ...
     if (!isset($_SESSION['sc_session']))
     {
       ...
       if (is_file($root . $_SESSION['scriptcase']['sec_grid_sec_users']['glo_nm_path_imag_temp'] . "/sc_apl_default_skini.txt"))
       {
?>
...
<?php
     ...
     if (!function_exists("NM_is_utf8"))
     {
      ...
      $Sel_Cmp = new sec_grid_sec_users_sel_cmp($bol_sel_campos_include); 
      $Sel_Cmp->Sel_cmp_init();
      $Sel_Cmp->Sel_cmp_init_fields();
      $Sel_Cmp->Sel_cmp_process();
     
  }
class sec_grid_sec_users_sel_cmp
{
...
function Sel_cmp_process()
{
   ...
   function Sel_cmp_init_fields()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['php_cmp_sel']))
      {
          foreach ($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
          {
              if ($NM_cada_opc == "off")
              {
                 $tab_ger_campos[$NM_cada_field] = "none";
      ...
      if ($this->restore)
      {
          if (isset($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel_orig']) && !empty($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel_orig']))
          {
              foreach ($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel_orig'] as $NM_cada_field => $NM_cada_opc)
              {
      ...
      {
          if (isset($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel']))
          {
              foreach ($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
              {
                  $tab_ger_campos[$NM_cada_field] = $NM_cada_opc;
              }
          }
...
function Sel_processa_out()
{
   ...
   $arr_temp = array();
   if (!isset($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel']))
   {
       $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel'] = array();
   }
   if($this->bol_sel_campos_include)
   {
       $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['export_sel_columns']['field_order'] = $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order'];
       $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['export_sel_columns']['usr_cmp_sel'] = $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel'];
   }
   $arr_order = $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order'];
   $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order'] = array();
   $campos_sel = explode("@?@", $this->campos_sel);
   foreach ($campos_sel as $campo_order)
   {
       if (isset($tab_ger_campos[$campo_order]))
       {
           $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order'][] = $campo_order;
       }
   }
   foreach ($arr_order as $campo_order)
   {
       if (!in_array($campo_order, $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order']))
       {
           $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order'][] = $campo_order;
       }
   }
   ...
   $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel'] = $arr_temp;
   if($this->bol_sel_campos_include)
   {
       return;
   }
...
?>
   $.ajax({
    type: "POST",
    url: "sec_grid_sec_users_sel_campos.php",
    data: {
...
<?php
   ...
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if ($NM_cada_opc != "on")
           {
               $prep_combo[strtolower($NM_cada_field)] = $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['labels'][$NM_cada_field];
           }
       }
   }
   if ($this->restore)
...
<?php
   ...
   if ($this->restore && isset($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order_orig']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order_orig'] as $NM_cada_field)
       {
...
?>
                        <li class="sc_ui_litem <?php echo $str_class; ?>" id="sc_id_itemsel_<?php echo NM_encode_input($NM_cada_field); ?>"><?php echo $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['labels'][$NM_cada_field]; ?></li>
<?php
           }
       }
   }
   ...
   if (!$this->restore && isset($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order'] as $NM_cada_field)
       {
           if ($tab_ger_campos[$NM_cada_field] == "on")
           {
               $str_class = "scAppDivSelectFieldsDisabled";
...
<HEAD>
 <TITLE><?php echo $this->Nm_lang['lang_list_users'] ?></TITLE>
...
<?php
  $disp_rest = "none";
  if (isset($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel']) && isset($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel_orig']) && $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel'] != $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel_orig']) {
      $disp_rest = "";
  }
  else {
      $order_orig = array();
      $order_atu  = array();
      foreach ($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order_orig'] as $ix => $cmp) {
          if (!isset($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel_orig'][$cmp]) || $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel_orig'][$cmp] != "off") {
              $order_orig[] = $cmp;
          }
      }
      foreach ($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['field_order'] as $ix => $cmp) {
          if (!isset($_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel'][$cmp]) || $_SESSION['sc_session'][$this->sc_init]['sec_grid_sec_users']['usr_cmp_sel'][$cmp] != "off") {
              $order_atu[] = $cmp;
          }
      }
