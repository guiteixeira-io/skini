...
*/
    ...
    $SC_cod_proj = "skini";
    $SC_apl_proc = "sec_grid_sec_users_groups";
    $SC_ver_93   = true;
...
*/
    $fonte        = (isset($_GET['conf_fonte']))    ? strip_tags($_GET['conf_fonte']) : "0";
    $grafico      = (isset($_GET['grafico']))       ? strip_tags($_GET['grafico']) : "";
    $largura      = (isset($_GET['largura']))       ? strip_tags($_GET['largura']) : "";
    $opc          = (isset($_GET['nm_opc']))        ? strip_tags($_GET['nm_opc']) : "";
    $target       = (isset($_GET['nm_target']))     ? strip_tags($_GET['nm_target']) : "";
    $language     = (isset($_GET['language']))      ? strip_tags($_GET['language']) : "en_us";
    $papel        = (isset($_GET['papel']))         ? strip_tags($_GET['papel']) : "";
    $cor          = (isset($_GET['nm_cor']))        ? strip_tags($_GET['nm_cor']) : "";
    $orientacao   = (isset($_GET['orientacao']))    ? strip_tags($_GET['orientacao']) : "";
    $conf_larg    = (isset($_GET['conf_larg']))     ? strip_tags($_GET['conf_larg']) : "N";
    $conf_socor   = (isset($_GET['conf_socor']))    ? strip_tags($_GET['conf_socor']) : "N";
    $apapel       = (isset($_GET['apapel']))        ? strip_tags($_GET['apapel']) : "";
    $lpapel       = (isset($_GET['lpapel']))        ? strip_tags($_GET['lpapel']) : "";
    $is_chart_app = (isset($_GET['is_chart_app']))  ? 'Y' == strip_tags($_GET['is_chart_app']) : false;
...
/*    $export_ajax = (isset($_GET['export_ajax'])) ? filter_input(INPUT_GET, 'export_ajax', FILTER_SANITIZE_STRING) : 'N';
    ...
    $export_ajax = (isset($_GET['export_ajax'])) ? strip_tags($_GET['export_ajax']) : 'N';
...
// Charts
        ...
        $isMultiSeriesChart = $is_chart_app && (!isset($_SESSION['sc_session'][$script_case_init]['sec_grid_sec_users_groups']['summarizing_drill_down']) || !$_SESSION['sc_session'][$script_case_init]['sec_grid_sec_users_groups']['summarizing_drill_down']);
        $isAnalitic         = isset($_SESSION['sc_session'][$script_case_init]['sec_grid_sec_users_groups']['graf_opc_atual']) && 2 == $_SESSION['sc_session'][$script_case_init]['sec_grid_sec_users_groups']['graf_opc_atual'];

if ($ver_93 == "n")
{
          $sDisplay = !isset($_SESSION['sc_session']['sec_grid_sec_users_groups']['show_skip_charts_option']) ||
                     !$_SESSION['sc_session']['sec_grid_sec_users_groups']['show_skip_charts_option'] ||
                     (isset($_SESSION['sc_session'][$script_case_init]['sec_grid_sec_users_groups']['SC_Ind_Groupby']) && '_NM_SC_' == $_SESSION['sc_session'][$script_case_init]['sec_grid_sec_users_groups']['SC_Ind_Groupby']) ||
                     $isMultiSeriesChart ||
                     $isAnalitic ||
                     ($opc == 'pdf_det')
                     ? ' style="display: none"' : '';
...
<?php
        ...
        {
         $sDisplay =
//                    !isset($_SESSION['sc_session']['sec_grid_sec_users_groups']['show_skip_charts_option']) ||
//                    !$_SESSION['sc_session']['sec_grid_sec_users_groups']['show_skip_charts_option'] ||
                    $isMultiSeriesChart ||
                    $isAnalitic ||
